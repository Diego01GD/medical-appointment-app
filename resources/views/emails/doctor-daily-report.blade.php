...
<body>
    <div class="container">
        <div class="header">
            <h1>Healthify</h1>
            <p>Tu Agenda Diaria de Citas</p>
        </div>

        <div class="content">
            <h2>¡Hola, Dr(a). {{ $doctor->user->name }}!</h2>
            <p>Este es el reporte automático con tu agenda programada para el día de hoy (<strong>{{ \Carbon\Carbon::today()->format('d/m/Y') }}</strong>).</p>
            
            <div class="info-card">
                <strong>Pacientes programados hoy:</strong> {{ $appointments->count() }}
            </div>
            ...
            @else
                <p>No tienes citas programadas para el día de hoy. ¡Disfruta tu día libre de consultas!</p>
            @endif

            <p style="margin-top: 30px; font-size: 14px; color: #666;">Te recordamos mantener actualizada la información de las consultas en el sistema. ¡Que tengas un excelente día!</p>
        </div>
